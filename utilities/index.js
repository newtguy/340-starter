...
/* **************************************
 * Build the recent view HTML
 * ************************************ */
Util.buildRecentGrid = async function (data) {
  let grid
  ...
  if (data.length > 0) {
    ...
    data.forEach((vehicle) => {
      ...
      grid +=
        "<span>Viewed: " +
        new Date(vehicle.viewed_at).toLocaleDateString() +
        "</span>"
    ...
    grid = '<p class="notice">You have not viewed any vehicles yet.</p>'
  }
